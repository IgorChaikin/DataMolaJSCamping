  ...
  return {
    getMessages(skip = 0, top = 10, filterConfig) {
      return messages.sort((a, b) => a.createdAt - b.createdAt)
        .slice(skip, skip + top)
        .filter(
          (message) => !filterConfig || (/* filter can be empty */
    ...
    validateMessage(msg) {
      const {
        id, text, createdAt, author, isPersonal, to,
      } = msg;
    ...
    addMessage(msg) {
      const isValid = this.validateMessage(msg);
      if (isValid) {
    ...
    editMessage(id, msg) {
      ...
      const isValid = this.validateMessage(edited);
      if (isValid) { messages[index] = edited; }
      return isValid;
    },
